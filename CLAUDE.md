...
**M0–M18 are done — the v1 roadmap (M0–M10) is complete, plus M11 keyboard input, M12 vehicle wheels, M12 HUD components, M12 collision, M13 particles, M14 breaking objects, M15 frame cost, M16 environment (sky, fog, shadows, MSAA, transparency), M17 fire + point lights, and M18 procedural trees** (and most of M1's CLI; M7 at scope E0–E2 + validation panel + --watch).
...
# Tree component (M18): a recipe, not a mesh — seeded procedural trunk/branches/leaves grown
...
Trees (M18, design in `tree-design.md`). The `Tree` component is a **recipe, not a mesh
...
since M16 the water and ice (real `Material.transmission`, not opaque stand-ins), since M17 the
campfire — layered additive flame, turbulent smoke, streaked embers, and a `PointLight` the tour's
effects script flickers off the same signal that drives the emission rates — and since M18 the
forest: nine `Tree` components (two oaks, a birch, three spruces, a snag, two scrubs) replacing the
twelve cylinder-and-sphere entities station 01 used to be, which cost all six showcase baselines a
re-bless and no other baseline anything. What is still faked and
...
lights, and a light on the tour's explosion (~~lights a script can drive~~ landed in M17); after
M18, texture-mapped materials — bark and alpha-cut leaves are the same missing feature — plus tree
LOD and wind.
