...
**M0–M25 are done** — the v1 roadmap (M0–M10) is complete, plus M11 keyboard input, M11.5 vehicle
...
M22 terrain, M23 roads, M24/M25 agent ergonomics. (M7 editor at scope E0–E2 + validation panel +
`--watch`.)
...
Component quick-reference (details below): `Script` runs `fn step(world, step)` once per fixed step
...
**15 of the 26 baselines are pinned by no test at all** (`m4_lighting`, both `m8_drop`, `m9_t025`,
...
texture-mapped materials — bark and alpha-cut leaves are the same missing feature — plus tree LOD and
wind. After M23: road junctions (two roads crossing wants a patch primitive, not a ribbon), banked
