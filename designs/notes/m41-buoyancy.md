# A CPU wave evaluator, and buoyancy (M41)
...
scenery. M41 writes the Rust mirror, holds it to the shader with a GPU agreement test, and spends it
on `Buoyancy`.
...
are handed the time it **ends** at (`step · dt`). That offset predates M41 — `simulate.rs` documents
...
`verify/m41_buoyancy.json` at `--steps 480` is **bit-exact** — five renders came back as one image —
