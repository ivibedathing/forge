...
The tour shows M28's `Meadow` too: a 24 × 14 m field of ground cover in front of station 04, on a
...
where before M28 only `showcase_646` had a tolerance at all. The strict pin on the meadow system
lives in `verify/m28_meadow.json` instead, which renders at `samples: 1` for exactly this reason.
